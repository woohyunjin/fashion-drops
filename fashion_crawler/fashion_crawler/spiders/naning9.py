...
from fashion_crawler.loader import ProductLoader
...
class Naning9_Spider(CrawlSpider):
	...
	rules = (
		Rule(LinkExtractor(allow=('shop\/shopbrand\.html', ))),
		Rule(LinkExtractor(allow=('shop\/shopdetail\.html', )), callback='parse_item'),
	)

	def parse_item(self, response):
		loader = ProductLoader(item=FashionCrawlerItem(), response=response);
		...
		loader.add_value('image_url', "http://www.naning9.com");
